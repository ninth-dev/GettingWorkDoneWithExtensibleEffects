...
object Scanner {
  ...
  implicit val s: Scheduler = Scheduler(ExecutionModel.BatchedExecution(32))
  ...
  def scanReport[R: _task: _filesystem: _err: _log](args: Array[String]): Eff[R, String] =
    for {
      ...
      topNValid <- {
        if (topN < 0)
          left[R, String, Int](s"Invalid number of files $topN")
        else
          topN.pureEff[R]
      }
      ...
      startTime <- taskDelay(System.currentTimeMillis())

      scan <- pathScan[Fx.prepend[Reader[ScanConfig, ?], R]](fs.filePath(base))
        .runReader[ScanConfig](ScanConfig(topNValid))

      finishTime <- taskDelay(System.currentTimeMillis())

      _ <- tell(Log.info(s"Scan of $base completed in ${finishTime - startTime}ms"))

    } yield ReportFormat.largeFilesReport(scan, base.toString)

  def pathScan[R: _task: _filesystem: _config: _log](path: FilePath): Eff[R, PathScan] =
    path match {
