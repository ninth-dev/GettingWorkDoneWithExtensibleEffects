...
object Scanner {
  ...
  def main(args: Array[String]): Unit = {
    val program: Eff[R, Unit] = scanReport[R](args).map(println)

    program
      .runReader(DefaultFilesystem: Filesystem)
      .evalStateZero[Set[FilePath]]
      .runEither
      .runWriterUnsafe[Log] {
  ...
  def scanReport[R: _task: _filesystem: _err: _log: _sym](args: Array[String]): Eff[R, String] =
    for {
      ...
      topNValid <- if (topN < 0) left[R, String, Int](s"Invalid number of files $topN")
      else topN.pureEff[R]
  ...
  def pathScan[R: _task: _filesystem: _config: _log: _sym](path: FilePath): Eff[R, PathScan] =
    path match {
      case Symlink(_,linkTo) =>
        for {
          linksVisited <- get[R, Set[FilePath]]
          scan <- if (!linksVisited.contains(linkTo)) {
            // we could also use modify which takes a function to modify the state
            // the >> is an operator (alias) for flatmap(_ => fb)
            put[R, Set[FilePath]](linksVisited + linkTo) >> pathScan(linkTo)
          } else {
      ...
      case f: File =>
        for {
          ...
          _ <- tell(
            Log.debug(s"File ${fs.file.path} Size ${ReportFormat.formatByteString(fs.size)}")
          )
      ...
      case dir: Directory =>
        for {
          ...
          _ <- {
            ...
            tell(
              Log.debug(s"Scanning directory '$dir': $dirCount subdirectories and $fileCount files")
            )
          }
